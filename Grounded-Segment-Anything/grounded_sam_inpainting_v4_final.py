...
v4：在 v3 全部改动基础上，新增 LaMa 替换后端 + 形态学文字预处理
...
v4 新增改进：
  ...
  ✅ 改进8：形态学文字预处理（object_removal 专用，不依赖 PaddleOCR）
           - 基于 OpenCV 形态学连通域分析，在 mask 区域内定位深色文字像素
           - 将"文字+气泡→背景"的复合任务拆分为两步：
             Step1: 形态学填白文字（简单）→ Step2: LaMa 修复气泡（单一）
           - 不依赖任何 OCR 框架，兼容性强，对漫画黑色文字效果稳定

v4 修复：
  ✅ 修复A：task_type 字符串统一为 object_removal / object_replacement
  ✅ 修复B：effective_inpaint_mode 仅对 removal 自动切换 merge，
            replacement 保持 first，避免多目标合并成超大 mask
  ✅ 修复C：LaMa 调用接口适配 IOPaint 实际 API
  ✅ 修复D：DSP 线条增强仅用于 SAM 分割输入，
...
def run_single_test(
    ...
    try:
        ...
        # ✅ 改进8：OCR/形态学文字预处理（object_removal + LaMa 模式下生效）
        if effective_ocr:
            print("  [改进8] 形态学文字预处理...")
            image_cv_for_inpaint = ocr_fill_text(
                image_np=image_cv_for_inpaint_base,
            ...
            print(f"  ✓ 形态学预处理完成，已保存 ocr_prefilled.jpg")
        else:
            image_cv_for_inpaint = image_cv_for_inpaint_base
        ...
        if use_lap_blend:
            ...
            blend_base = image_cv_for_inpaint if effective_ocr else image_cv_for_inpaint_base
