...
v4：在 v3 全部改动基础上，新增 LaMa 替换后端 + OCR 文字预处理

v4 新增改进：
  ✅ 改进7：LaMa Inpainting 后端（object_removal 专用，替代 SDXL）
           - LaMa 基于傅里叶卷积，专为"擦除还原"设计，对漫画网点/规律
             线条的重建质量远超 SDXL latent diffusion
           - object_removal → LaMa；object_replacement → SDXL（不变）
  ✅ 改进8：OCR 文字预处理（PaddleOCR，object_removal 专用）
           - 在 LaMa 修复前先用 OCR 精确定位文字区域并用白色填充
           - 将"文字+气泡→背景"的复合任务拆分为两步：
             Step1: OCR 填白文字（简单）→ Step2: LaMa 修复气泡（单一）
           - 大幅降低 inpainting 模型的重建难度
...
依赖安装（新增）：
  pip install iopaint paddlepaddle paddleocr
"""
...
# ✅ 改进8：PaddleOCR 可选导入——不强制，缺失时跳过 OCR 预处理步骤
try:
    ...
    print("⚠ PaddleOCR 未安装，OCR 预处理将被跳过（pip install paddlepaddle paddleocr）\n")
...
def lama_inpaint(image_np: np.ndarray,
    ...
    LaMa 不需要文本 prompt，纯靠周边像素上下文推断被遮挡区域。
    ...
    # IOPaint 要求 mask 为单通道，白色（255）为修复区
    mask_single = mask_np if mask_np.ndim == 2 else mask_np[:, :, 0]

    # 构造请求对象
    req = InpaintRequest(
        hd_strategy=HDStrategy.ORIGINAL,   # 保持原始分辨率，不做 HD 分块
    ...
    # 执行修复（返回 BGR uint8）
    result_bgr = model(image_np, mask_single, req)

    # IOPaint 内部使用 BGR，转回 RGB
    result_rgb = cv2.cvtColor(result_bgr, cv2.COLOR_BGR2RGB)
    return result_rgb.astype(np.uint8)


# ==================== ✅ 改进8：OCR 文字预处理模块 ====================
# 原代码（v3）：将"文字+气泡外框"的 mask 整体送入 inpainting，
#               模型需要同时"擦除文字"和"还原背景纹理"，任务复杂度高
# 新代码（v4）：先用 PaddleOCR 精确定位文字像素区域并填白，
#               再送入 LaMa 只面对"白色气泡→背景纹理"的单一任务，
#               大幅降低模型重建难度，边界更干净

# PaddleOCR 单例
_ocr_engine = None

def get_ocr_engine(use_gpu: bool = True):
    """
    懒加载 PaddleOCR 引擎（全局单例）
    Args:
        use_gpu: 是否使用 GPU 加速
    Returns:
        PaddleOCR 实例
    ...
    if _ocr_engine is None:
        if not PADDLEOCR_AVAILABLE:
            raise RuntimeError(
                "PaddleOCR 未安装，无法使用 OCR 预处理。"
                "请运行：pip install paddlepaddle paddleocr"
            )
        print("  [改进8] 首次初始化 PaddleOCR 引擎（后续调用复用）...")
        _ocr_engine = PaddleOCR(lang="ch")
        print("  ✓ PaddleOCR 引擎加载完成")
    return _ocr_engine
...
def ocr_fill_text(image_np: np.ndarray,
    ...
    """
    ✅ 改进8（v4 修订版）：基于形态学的文字区域检测与填充
    放弃 PaddleOCR（版本兼容性问题），改用 OpenCV 形态学方法：
    1. 在 mask 区域内提取深色像素（漫画文字通常为黑色）
    2. 用形态学操作连接相邻文字像素，形成文字连通域
    3. 按连通域面积过滤（太大=背景线条，太小=噪点），保留文字尺寸的连通域
    4. 将识别到的文字区域填充为 fill_color

    对漫画黑色文字效果稳定，不依赖任何 OCR 框架。
    """
    image_filled = image_np.copy()
    mask_binary  = (mask_np > 127).astype(np.uint8)

    # Step 1：在 mask 区域内提取深色像素（文字通常为接近黑色的深色）
    gray = cv2.cvtColor(image_np, cv2.COLOR_RGB2GRAY)
    # 阈值：灰度值 < 80 认为是深色文字（可根据实际漫画调整）
    _, dark_mask = cv2.threshold(gray, 80, 255, cv2.THRESH_BINARY_INV)
    # 只保留 mask 区域内的深色像素
    text_candidates = cv2.bitwise_and(dark_mask, dark_mask,
                                      mask=mask_binary)
...
def run_single_test(
    ...
    try:
        ...
        # object_replacement 仍走 SDXL（需要语义生成能力）
        ...
        # ✅ 改进8：OCR 文字预处理（object_removal + LaMa 模式下生效）
        # 原代码（v3）：直接将含文字的气泡区域送入 inpainting，任务复杂度高
        # 新代码（v4）：先用 OCR 填白 mask 区域内的文字，
        #               LaMa 只需面对"白色气泡→背景"的单一任务
        if effective_ocr:
            print("  [改进8] OCR 文字预处理...")
            image_cv_for_inpaint = ocr_fill_text(
                image_np=image_cv_enhanced,
                mask_np=merged_mask_np,
                fill_color=ocr_fill_color,
                ocr_confidence_threshold=ocr_confidence_threshold,
                # use_gpu=(device == "cuda"),
            )
            # 保存 OCR 填充后的图像供调试对比
            Image.fromarray(image_cv_for_inpaint).save(
                os.path.join(output_dir, "ocr_prefilled.jpg"))
            image_pil_for_inpaint = Image.fromarray(image_cv_for_inpaint)
            print(f"  ✓ OCR 预处理完成，已保存 ocr_prefilled.jpg")
        else:
            image_cv_for_inpaint = image_cv_enhanced

        original_size = image_pil_for_inpaint.size
        # LaMa 分支不需要 padding，new_wh/offset 仅 SDXL 分支内部使用
        # 此处给默认值避免变量未定义报错（SDXL 分支内会覆盖）
        ...
        # [6/8] Inpainting（v4：LaMa 分支 / SDXL 分支）
        print("  [6/8] 运行 Inpainting...")

        # ✅ 改进7：object_removal 优先走 LaMa，object_replacement 走 SDXL
        # 原代码（v3）：两类任务都调用 SDXL pipe，只通过 prompt 区分
        # 新代码（v4）：
        #   object_removal  → LaMa（无需 prompt，专为擦除设计，不产生幻觉）
        #   object_replacement → SDXL（需要语义生成，保持不变）
        if effective_lama:
            print("  [改进7] 使用 LaMa 执行 object_removal inpainting...")
            # LaMa 直接接收原始尺寸图像和 mask，不需要 padding 到 1024
            inpainted_np_lama = lama_inpaint(
                image_np=np.array(image_pil_for_inpaint),
                mask_np=merged_mask_np,
                device=device,
            )
            inpainted_image = Image.fromarray(inpainted_np_lama)
            # 保存 LaMa 原始输出供调试
            inpainted_image.save(os.path.join(output_dir, "lama_raw_output.jpg"))
            print("  ✓ LaMa inpainting 完成")

            # LaMa 输出已是原始尺寸，无需 restore_from_padding
            inpainted_np = inpainted_np_lama

        else:
            # ── SDXL 分支（object_replacement 或 LaMa 不可用时的降级） ──
            image_padded, new_wh, offset = resize_with_padding(
                image_pil_for_inpaint, target_size=1024)
            ...
            if use_controlnet:
                ...
                condition_image.save(
                    os.path.join(output_dir,
                                 f"controlnet_condition_{controlnet_condition_type}.jpg"))
        ...
        if use_lap_blend:
            ...
            # ✅ 改进8（v4）：融合基准图改为 OCR 预处理后的图像（若 OCR 生效）
            # 原代码（v3）：始终用 image_cv_enhanced 作为融合基准
            # 新代码（v4）：若 OCR 预处理生效，用 image_cv_for_inpaint（已填白文字）
            #               保证融合基准与 inpainting 输入一致，避免文字残影
            blend_base = image_cv_for_inpaint if effective_ocr else image_cv_enhanced
        ...
        if effective_color:                           # ✅ 改进5：跟随 task_type 自动决策
            ...
            blended_np = color_match_inpainted(
                orig_np=blend_base,               # ✅ 改进8（v4）：与融合基准保持一致
