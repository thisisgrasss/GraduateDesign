...
try:
    ...
    from grounded_sam_inpainting import (
        load_image,
        load_model,
        get_grounding_output,   # 注意：改进版现在返回 (boxes, phrases, scores) 三个值
        filter_boxes_nms,       # ✅ 改动6：NMS 过滤函数
    ...
    from PIL import Image
    from segment_anything import SamPredictor, build_sam
    # ✅ 改动5：换用 SD 2 inpainting，pipeline 类不变，只改模型名
    # from diffusers import StableDiffusionInpaintPipeline
...
CONFIG = {
    "grounding_dino_config": "Grounded-Segment-Anything/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py",
    ...
    "image_path": "Grounded-Segment-Anything/dataset/AmericanComic/cartoon/1.jpg",
    "det_prompt": "fan",
    "inpaint_prompt": "cartoon style, vibrant colors, high quality, detailed, Handbag standing upright",
    "output_dir": "single_test_output",
    "box_threshold": 0.3,
    "text_threshold": 0.25,
    "device": "cuda",
    ...
    # ✅ 改动3：新增 inpainting 推理参数配置项，方便统一调整
    "inpaint_steps": 50,
...
def main():
    ...
    print("步骤 1/7: 加载 Grounding DINO 模型...")
    try:
        model = load_model(config["grounding_dino_config"], config["grounded_checkpoint"], device=device)
        print("✓ Grounding DINO 模型加载完成\n")
    except Exception as e:
        print(f"✗ 模型加载失败: {e}")
        sys.exit(1)
    ...
    # ✅ 改动6：get_grounding_output 现在返回三个值，增加 scores 接收
    ...
    # ✅ 改动6：新增 NMS 过滤，去除重叠检测框
    # 原代码：无此步骤
    boxes_filt, pred_phrases, scores = filter_boxes_nms(
        boxes_filt, pred_phrases, scores,
        iou_threshold=config["nms_iou_threshold"]
    )

    print(f"✓ 检测到 {len(boxes_filt)} 个目标（NMS过滤后）")
    if len(boxes_filt) > 0:
        print(f"  检测结果: {pred_phrases}\n")
    else:
        print("  未检测到任何目标!\n")
    ...
    # ✅ 改动4：使用更健壮的 mask 合并方式（torch.any 替代 torch.sum）
    # 原代码：mask = masks[0][0].cpu().numpy()，直接取第一个，忽略多目标情况
    if len(masks) > 1:
    ...
    # ✅ 改动2：保持宽高比 resize，替代原来的强制 512x512
    # 原代码：image_pil_resized = image_pil_resized.resize((512, 512))
    #         mask_pil = mask_pil.resize((512, 512))
    ...
    inpainted_image = pipe(
        prompt=config["inpaint_prompt"],
        negative_prompt=config["inpaint_negative_prompt"],   # ← 改动3
        image=image_padded,
        mask_image=mask_padded,
        num_inference_steps=config["inpaint_steps"],         # ← 改动3
        guidance_scale=config["inpaint_guidance_scale"],     # ← 改动3
        strength=config["inpaint_strength"],                 # ← 改动3
    ).images[0]

    # ✅ 改动2：还原 padding，resize 回原始尺寸
    # 原代码：inpainted_image = inpainted_image.resize(size)
    ...
    if config["enable_metrics"]:
        ...
        try:
            evaluator = Evaluator(device)
            metrics = evaluator.run(raw_image_path, inpainting_output_path, config["inpaint_prompt"])
            print("✓ 评价指标计算完成\n")
        except Exception as e:
            print(f"✗ 评价指标计算失败: {e}")
            print("继续执行，但无评价指标结果\n")
    ...
    if metrics:
        ...
        print(f"\n指标解读:")
        print(f"  • 结构保持: {'优秀' if metrics['SSIM'] >= 0.9 else '良好' if metrics['SSIM'] >= 0.8 else '一般'} (SSIM={metrics['SSIM']})")
        print(f"  • 感知质量: {'优秀' if metrics['LPIPS'] <= 0.2 else '良好' if metrics['LPIPS'] <= 0.4 else '一般'} (LPIPS={metrics['LPIPS']})")
        print(f"  • 文本匹配: {'优秀' if metrics['CLIP_Score'] >= 30 else '良好' if metrics['CLIP_Score'] >= 20 else '一般'} (CLIP={metrics['CLIP_Score']})")
        ...
        with open(metrics_path, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "det_prompt": config["det_prompt"],
                "inpaint_prompt": config["inpaint_prompt"],
                "inpaint_model": "stabilityai/stable-diffusion-2-inpainting",  # ✅ 改动5：记录新模型名
                "inpaint_steps": config["inpaint_steps"],                       # ✅ 改动3：记录推理参数
                "inpaint_guidance_scale": config["inpaint_guidance_scale"],
                "inpaint_strength": config["inpaint_strength"],
                "detected_objects": len(boxes_filt),
                "detected_labels": pred_phrases,
                "metrics": metrics
            }, f, indent=2, ensure_ascii=False)
